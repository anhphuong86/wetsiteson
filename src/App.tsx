...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubAuth = auth.onAuthStateChanged((u) => {
      ...
      setIsAdmin(u?.email === 'letuananhphuong@gmail.com');
    });
  ...
  const estimation = useMemo(() => {
    ...
    if (projType === 'wall') {
      ...
      return {
        realArea,
        items: [
          { name: chosenPutty?.name[lang], qty: puttyBags, unit: 'Bao', cost: puttyCost },
          { name: chosenPrimer?.name[lang], qty: primerBuckets.formatted, unit: 'Hệ', cost: primerBuckets.cost },
          { name: chosenTopcoat?.name[lang], qty: topcoatBuckets.formatted, unit: 'Hệ', cost: topcoatBuckets.cost }
        ].filter(i => i.name),
        labor: laborCost,
                 ...
                 <div className="flex justify-between items-end mb-12">
                   ...
                   <div className="flex gap-4">
                     {dbProducts.length === 0 && (
                       <button 
                         onClick={async () => {
                           if (!window.confirm('Bạn có muốn nhập dữ liệu sản phẩm mẫu vào hệ thống không?')) return;
        ...
        <div className="max-w-7xl mx-auto glass-card rounded-[2.5rem] h-20 px-10 flex items-center justify-between border border-white/40 shadow-2xl relative overflow-hidden group">
          ...
          <div className="flex items-center gap-4 relative z-10">
            {!user && (
              <button 
                onClick={handleLogin}
                className="p-3 text-brand-charcoal/40 hover:text-brand-green transition-all group"
                title="Staff Login"
              >
        ...
        <section id="story" className="py-24 md:py-48 bg-white relative overflow-hidden bg-vector-pattern">
          ...
          <div className="section-container relative z-10">
            <div className="text-center mb-32 max-w-4xl mx-auto relative">
              ...
              <p className="text-xl md:text-3xl text-brand-charcoal/50 leading-relaxed font-medium">
                {t.visionDesc}
              </p>
            </div>
            ...
            <div className="relative py-20 pointer-events-none overflow-hidden">
              <svg className="w-full h-24 text-brand-accent/5 fill-current" viewBox="0 0 1440 320" preserveAspectRatio="none">
                <path d="M0,192L48,197.3C96,203,192,213,288,229.3C384,245,480,267,576,250.7C672,235,768,181,864,181.3C960,181,1056,235,1152,234.7C1248,235,1344,181,1392,154.7L1440,128L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" />
              </svg>
              <div className="absolute top-1/2 left-0 w-full flex justify-around opacity-10">
                {[...Array(12)].map((_, i) => (
                  <div key={i} className="w-2 h-2 rounded-full bg-brand-accent" />
                ))}
              </div>
            </div>

            <div className="grid lg:grid-cols-3 gap-16 items-start mt-32">
              <div className="space-y-12">
                ...
                {FEATURES_7[lang].slice(0, 3).map((f, i) => (
                  <div key={i} className="group pr-0 md:pr-12">
                    <div className="flex items-center gap-4 mb-3">
                      ...
                      <h4 className="text-xl font-bold tracking-tight text-brand-charcoal leading-tight">{f}</h4>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-12">
                ...
                {FEATURES_7[lang].slice(3).map((f, i) => (
                  <div key={i} className="group pl-0 md:pl-12">
                    <div className="flex items-center gap-4 mb-3">
          ...
          <div className="section-container">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
              {(dbCertificates.length > 0 ? dbCertificates : [
                { id: 'c1', title: 'Chứng nhận ISO 9001', imageUrl: 'https://images.unsplash.com/photo-1633158829585-23ba8f7c8caf?q=80&w=2070&auto=format&fit=crop' },
                { id: 'c2', title: 'Chứng nhận JIS K 5674', imageUrl: 'https://images.unsplash.com/photo-1579389083078-4e7018379f7e?q=80&w=2070&auto=format&fit=crop' },
                { id: 'c3', title: 'Hợp chuẩn Hợp quy', imageUrl: 'https://images.unsplash.com/photo-1450101499163-c8848c66ca85?q=80&w=2070&auto=format&fit=crop' },
                { id: 'c4', title: 'Chứng nhận Green Label', imageUrl: 'https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?q=80&w=2013&auto=format&fit=crop' },
              ]).map((cert, index) => (
                <motion.div
                  key={cert.id}
                          ...
                          <table className="w-full text-left border-collapse mb-12 text-black">
                            <thead>
                              <tr className="bg-gray-100 uppercase text-[10px] font-black tracking-widest text-gray-600">
                                ...
                                <th className="p-4 border border-gray-200">Khối lượng</th>
                                <th className="p-4 border border-gray-200">ĐVT</th>
                                <th className="p-4 border border-gray-200 text-right">Thành tiền (VNĐ)</th>
                              </tr>
                            </thead>
